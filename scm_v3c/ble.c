...
void ble_generate_packet(void) {
    ...
    memcpy(&ble_vars.packet[i], pdu_crc, BLE_PDU_LENGTH + BLE_CRC_LENGTH + 5);
		
		i += (BLE_PDU_LENGTH + BLE_CRC_LENGTH);
    // ---CTE------------------------------------------------------
    // Use 0xff to fill the packet for I/Q measurement
